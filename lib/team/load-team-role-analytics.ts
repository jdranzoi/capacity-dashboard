...
import { overviewWeeklyLoggedUtilizationPct } from '@/lib/domain/workload-metrics'
...
export async function loadTeamRoleAnalytics(
  ...
  try {
    const [capRes, planRes, wlRes] = await Promise.all([
    ...
    for (const [k, agg] of byRole) {
      ...
      tmp.push({
        ...
        row: {
          ...
          utilizationPct: overviewWeeklyLoggedUtilizationPct(loggedRounded, netRounded),
        },
      })
    }
